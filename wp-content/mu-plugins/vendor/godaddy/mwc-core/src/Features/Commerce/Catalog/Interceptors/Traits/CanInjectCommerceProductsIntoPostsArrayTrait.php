...
trait CanInjectCommerceProductsIntoPostsArrayTrait
{
    ...
    protected function injectCommerceData(array $posts, ?array $localIds = null) : array
    {
        ...
        $remoteProducts = $this->batchListProductsByLocalIdService->batchListByLocalIds($localIds);
    ...
    protected function shouldQueryPlatformForProducts() : bool
    {
         ...
         *
         * When {@see url_to_postid()} runs it triggers a WP_Query, and that can lead to an infinite loop (see issue above).
         * Skipping an API request in this instance should be fairly safe as:
         *
         *   1. WordPress core doesn't call this function during a typical page request; and
         *   2. When it _does_ get called, overlaying Commerce data isn't particularly helpful, as the very nature of
         *      "URL to post ID" means we really just want to check the local database. We don't read URL slugs from the API.
         *
         * If we end up seeing N+1 issues we may have to reevaluate this check.
         */
        return did_filter('url_to_postid') === 0 && CatalogIntegration::hasCommerceCapability(Commerce::CAPABILITY_READ);
    }

...
trait CanCrudPlatformInventoryDataTrait
{
    ...
    {
        $nativeProduct = Product::getNewInstance()->setId($product->get_id());

        try {
            $foundSummary = $this->summariesService->readSummary(new ReadSummaryOperation($nativeProduct))->getSummary();
